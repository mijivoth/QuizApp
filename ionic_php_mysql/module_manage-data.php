  ...
  header('Access-Control-Allow-Origin: *');
   ...
   switch($key)
   {
      ...
      case "insert":
         ...
         $module_number 		     = filter_var($obj->module_number, FILTER_SANITIZE_NUMBER_INT);
         $module_name 		     = filter_var($obj->module_name, FILTER_SANITIZE_STRING, FILTER_FLAG_ENCODE_LOW);
         


         // Attempt to run PDO prepared statement
         try {
            $sql 	= "INSERT INTO module(which_lecture, module_number, module_name) VALUES(:lecture_name, :module_number,:module_name)";
            $stmt 	= $pdo->prepare($sql);
            $stmt->bindParam(':lecture_name', $lecture_name, PDO::PARAM_STR);
            $stmt->bindParam(':module_number', $module_number, PDO::PARAM_INT);
            $stmt->bindParam(':module_name', $module_name, PDO::PARAM_STR);
            $stmt->execute();

            echo json_encode(array('message' => 'Congratulations the record ' . $module_name . ' was added to the database'));
         }
